...
@dataclass
class HidePromptConfig(PromptLearningConfig):
    """Configuration for HiDe-Prompt (prompt-pool) in NLP models.

    Fields align with EPrompt's __init__ while keeping PromptLearningConfig contract.
    """
    ...
    batchwise_prompt: bool = field(default=False, metadata={"help": "Batch-wise prompt selection"})
...
class EPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=None, embedding_key='mean', prompt_init='uniform', prompt_pool=True, 
        ...
        # HiDe-Prompt key initialization (usually not used)
        if prompt_key:
            ...
            # HiDe-Prompt default: no learnable keys, selection relies on external prompt_mask/prompt_idx
            # Set prompt_key to None to indicate external selection is used
