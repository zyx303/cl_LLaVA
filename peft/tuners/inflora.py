...
class LoraLayer:
    def __init__(
        ...
        # InfLoRA specific
        assert in_features == out_features, "InfLoRA only supports in_features == out_features"
        self.matrix = torch.zeros(in_features, out_features)
        self.n_matrix = 0
        self.cur_matrix = torch.zeros(in_features, out_features)
        self.n_cur_matrix = 0
