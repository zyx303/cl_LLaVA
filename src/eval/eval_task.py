...
import random
def deadlock_detection(vis_feats, actions, window_size=10):

    # not enough vis feats for loop detection
    if len(vis_feats) < window_size*2:
        return False, None

    nav_actions = ['MoveAhead_25', 'RotateLeft_90', 'RotateRight_90']
...
class EvalTask(Eval):
    ...
    '''
    @classmethod
    def load_task_json(cls, task_path):
        # TODO
        pass

    @classmethod
    def run(cls, model, task_queue, args, lock, successes, failures, results):
        '''
        evaluation loop
        '''
        # start THOR
        ...
        while True:
            ...
            try:
                traj = cls.load_task_json(task)
                r_idx = task['repeat_idx']
                print("Evaluating: %s" % (traj['root']))
                print("No. of trajectories left: %d" % (task_queue.qsize()))
                cls.evaluate(env, model, r_idx, traj, args, lock, successes, failures, results)
    ...
    # r_idx: repeat idx 
    @classmethod
    def evaluate(cls, env, model, r_idx, traj_data, args, lock, successes, failures, results):
        seed = 123
        ...
        while not done:
            ...
            high_instr,vis_feat = cls.generate_response(curr_image, goal_instr) #high
            if os.getenv("DEBUG", "0") == "1":
                print("High-level instruction: %s" % high_instr)
            #TODO 复用vis_feature
            low_action = cls.generate_response(curr_image, goal_instr,vis_feature=None, high_instr=high_instr, model_type='low') # low
            vis_feats.append(vis_feat)
            if os.getenv("DEBUG", "0") == "1":
                print("Low-level action: %s" % low_action)
            # action prediction
            action = low_action['action']

            # Loop detection
            isLoop, rand_action = deadlock_detection(vis_feats, pred_actions, 10)
            if isLoop:
                action = rand_action
                loop_count += 1

            #TODO 当画面不变的时候，选择新的action
            ...
            # mask prediction, assuming we know mask
            mask = None
            if model.has_interaction(action):
                pred_class = low_action['object']

                out = {'boxes': [], 'labels': [], 'scores': [], 'masks': []}
                gt_masks = env.last_event.instance_masks
                for objectId in gt_masks:
                    object_class = objectId.split('|')
                    object_class = object_class[-1].split('_')[0] if len(object_class) > 4 else object_class[0]
                    if object_class in classes:
                        out['labels'].append(classes.index(object_class))
                        out['scores'].append(1)
                        out['masks'].append(gt_masks[objectId][np.newaxis,:,:])
                out['labels'] = torch.tensor(out['labels'], dtype=torch.long)
                out['scores'] = torch.tensor(out['scores'], dtype=torch.float)
                out['masks'] = torch.tensor(out['masks'], dtype=torch.float)

                if sum(out['labels'] == pred_class) == 0:
                    # TODO resample action and mask should be None
                    pass
                    # mask = None
